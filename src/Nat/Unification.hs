...
data UnificationError a
  = NotUnifiable !a !a
  | ContextualUnificationError (UnificationError a) (Env a)
  deriving (Eq)
...
type UnifyT a r = StateT (Env a) (Except (UnificationError a)) r

type UnifyM a = UnifyT a (Env a)

execUnifyT :: Env a -> UnifyT a r -> Either (UnificationError a) (Env a)
execUnifyT s m = runExcept (execStateT m s)

class (Substitutable a a, Show a, Pretty a, Ord a) => Unifiable a where
  unify :: a -> a -> UnifyM a

  -- | Simple unification doesn't need access to the
  ...
  updateSignatureFoo :: Pair a -> [Pair a] -> UnifyT a [Pair a]
  updateSignatureFoo (a, b) cs = do
    unifier <- unify a b
    signature <- gets (compose unifier)
    put signature
    let cs' = inEnv signature cs
  ...
  unifyManyFoo :: [Pair a] -> UnifyT a ()
  unifyManyFoo =
    expandInvariants >=> log >=> \case
      [] -> pure mempty
      (c : cs) -> updateSignatureFoo c cs >>= unifyMany
    where
      log cs = do
        traceM ("constraints:\n" ++ (show $ pp cs))
        return cs

  runUnifyIn :: Env a -> [Pair a] -> Either (UnificationError a) (Env a)
  runUnifyIn sig = execUnifyT sig . unifyMany

  runUnify :: [Pair a] -> Either (UnificationError a) (Env a)
  runUnify = runUnifyIn mkUState

  unifiableIn :: Env a -> [Pair a] -> Bool
  unifiableIn sig = isRight . execUnifyT sig . unifyManyFoo
